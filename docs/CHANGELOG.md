...
- **已知取舍**（文档已注明）：行间/字间 0 间隙（二值化后笔画相连）的文本上下字都会被拒识；正常 >=1px 间隙不受影响。右侧沿用历史宽松阈值（h/2），四边阈值刻意不对称。
- **精确路径状态坑修复**：guard 拒绝候选时必须清 `matched` 残留，否则该尺寸组最后一个候选被拒后残留 1 会跳出组循环，导致后续更大的超集本尊条目（如字库同装 大/太 时的"太"）失去匹配机会。
