...
WinKeyboard::WinKeyboard() : KeyboardBackend() {
}
...
long WinKeyboard::Bind(HWND hwnd, long mode) {
    if (!::IsWindow(hwnd))
        return 0;
    _hwnd = hwnd;
    _mode = mode;
...
long WinKeyboard::GetKeyState(long vk_code) {
    return (::GetAsyncKeyState(vk_code) & 0x8000) ? 1 : 0;
}

long WinKeyboard::KeyDown(long vk_code) {
    long ret = 0;
...
long WinKeyboard::KeyUp(long vk_code) {
    long ret = 0;
    // vk_code = toupper(vk_code);
    switch (_mode) {
    case INPUT_TYPE::IN_NORMAL: {
...
long WinKeyboard::WaitKey(long vk_code, unsigned long time_out) {
...
long WinKeyboard::KeyPress(long vk_code) {
    KeyDown(vk_code);
    switch (_mode) {
    case INPUT_TYPE::IN_NORMAL: {
