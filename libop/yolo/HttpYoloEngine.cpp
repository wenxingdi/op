...
HttpYoloEngine::HttpYoloEngine() : m_endpoint(resolve_default_endpoint()), m_timeout_ms(resolve_default_timeout_ms()) {
    if (!normalize_endpoint(m_endpoint, kYoloDefaultPathSuffix))
        m_endpoint = kYoloDefaultEndpoint;
    cout << "HttpYoloEngine::HttpYoloEngine(), endpoint=" << m_endpoint << endl;
}

HttpYoloEngine::~HttpYoloEngine() = default;

int HttpYoloEngine::init(const std::wstring &engine, const std::wstring &dllName, const vector<string> &argvs) {
...
int HttpYoloEngine::detect(byte *data, int w, int h, int bpp, double conf, double iou, vyolo_rec_t &result) {
    result.clear();
