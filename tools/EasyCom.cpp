...
HRESULT(__stdcall *oCLSIDFromProgID)(_In_ LPCOLESTR lpszProgID, _Out_ LPCLSID lpclsid);

HRESULT(__stdcall *oCoCreateInstance)
(REFCLSID rclsid, LPUNKNOWN pUnkOuter, DWORD dwClsContext, REFIID riid, LPVOID FAR *ppv);

HRESULT(__stdcall *oCoGetClassObject)
(_In_ REFCLSID rclsid, _In_ DWORD dwClsContext, _In_opt_ LPVOID pvReserved, _In_ REFIID riid, _Outptr_ LPVOID FAR *ppv);

HRESULT __stdcall myCLSIDFromProgID(_In_ LPCOLESTR lpszProgID, _Out_ LPCLSID lpclsid) {
...
HRESULT __stdcall myCoCreateInstance(_In_ REFCLSID rclsid, _In_opt_ LPUNKNOWN pUnkOuter, _In_ DWORD dwClsContext,
                                     _In_ REFIID riid,
...
HRESULT __stdcall myCoGetClassObject(_In_ REFCLSID rclsid, _In_ DWORD dwClsContext, _In_opt_ LPVOID pvReserved,
    ...
    if (memcmp(&rclsid, &CLSID_OpAutomation, sizeof(CLSID_OpAutomation)) == 0) {
        ...
        typedef HRESULT(__stdcall * DllGetClassObject_t)(_In_ REFCLSID rclsid, _In_ REFIID riid,
...
HRESULT(__stdcall *oLoadRegTypeLib)(REFGUID rguid, WORD wVerMajor, WORD wVerMinor, LCID lcid, ITypeLib **pptlib);
HRESULT __stdcall mLoadRegTypeLib(REFGUID rguid, WORD wVerMajor, WORD wVerMinor, LCID lcid, ITypeLib **pptlib) {
    ...
    HRESULT hr = memcmp(&rguid, &LIBID_opLib, sizeof(LIBID_opLib)) == 0
                     ? LoadTypeLib(dllpathW, pptlib)
                     : oLoadRegTypeLib(rguid, wVerMajor, wVerMinor, lcid, pptlib);
    return hr;
